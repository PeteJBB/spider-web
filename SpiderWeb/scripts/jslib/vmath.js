/* This file was generated from TypeScript source tslib/vmath.ts */
...
var VMath = {
    ...
    truncate: /*jshint bitwise: false*/
    function truncateFn(value) {
        return (value | 0);
    },
    v2BuildZero: /*jshint bitwise: true*/
    //
    // Vector2
    //
    function v2BuildZeroFn(dst) {
        if(dst === undefined) {
    ...
    v2MaskEqual: // Vector2 'masks'
    function v2MaskEqualFn(a, b) {
        var abs = Math.abs;
    ...
    v2ScalarBuild: // Vector2 operations with scalar
    function v2ScalarBuildFn(a, dst) {
        if(dst === undefined) {
            dst = new VMathArrayConstructor(2);
        }
        dst[0] = a;
    ...
    v2EqualScalarMask: // Vector2 'masks' with scalars
    function v2EqualScalarMaskFn(a, b) {
        var abs = Math.abs;
        var precision = VMath.precision;
        return [
            (abs(a[0] - b) <= precision), 
    ...
    v3ScalarBuild: // Vector3 operations with scalar
    function v3ScalarBuildFn(a, dst) {
        if(dst === undefined) {
            dst = new VMathArrayConstructor(3);
        }
        debug.assert(debug.isNumber(a));
    ...
    v4ScalarBuild: // Vector4 operations with scalar
    function v4ScalarBuildFn(a, dst) {
        if(dst === undefined) {
            dst = new VMathArrayConstructor(4);
        }
        debug.assert(debug.isNumber(a));
    ...
    m33ScalarAdd: // Matrix3 operations with scalar
    function m33ScalarAddFn(m, s, dst) {
        if(dst === undefined) {
            dst = new VMathArrayConstructor(9);
        }
        debug.assert(debug.isMtx33(m));
    ...
    m43ScalarAdd: // Matrix 43 opeations with scalar
    function m43ScalarAddFn(m, s, dst) {
        if(dst === undefined) {
            dst = new VMathArrayConstructor(12);
        }
        debug.assert(debug.isMtx43(m));
    ...
    m44ScalarAdd: // Matrix44 operations with scalars
    function m44ScalarAddFn(m, s, dst) {
        if(dst === undefined) {
            dst = new VMathArrayConstructor(16);
        }
        debug.assert(debug.isMtx44(m));
    ...
    // Visibility queries
    //
    function isVisibleBoxFn(center, halfDimensions, vpm) {
        var abs = Math.abs;
        var c0 = center[0];
        var c1 = center[1];
        var c2 = center[2];
    ...
    isVisibleSphere: function isVisibleSphereFn(center, radius, vpm) {
        ...
        var T3 = (m3 * c0 + m7 * c1 + m11 * c2 + vpm[15]);
        var nradius = -radius;
        return !(((T0 - T3) > radius * (abs(I0 - I3) + abs(J0 - J3) + abs(K0 - K3))) || ((T0 + T3) < nradius * (abs(I0 + I3) + abs(J0 + J3) + abs(K0 + K3))) || ((T1 - T3) > radius * (abs(I1 - I3) + abs(J1 - J3) + abs(K1 - K3))) || ((T1 + T3) < nradius * (abs(I1 + I3) + abs(J1 + J3) + abs(K1 + K3))) || ((T2 - T3) > radius * (abs(I2 - I3) + abs(J2 - J3) + abs(K2 - K3))) || ((T2 + T3) < nradius * (abs(I2 + I3) + abs(J2 + J3) + abs(K2 + K3))) || //((T3 - T3) >  radius * (abs(I3 - I3) + abs(J3 - J3) + abs(K3 - K3))) ||
    ...
    // Planes
    //
    function planeNormalizeFn(plane, output) {
        if(output === undefined) {
